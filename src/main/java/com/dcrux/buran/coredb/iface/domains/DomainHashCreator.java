...
public class DomainHashCreator {
  ...
  public DomainHashCreator(UUID uuid, String creatorName, String creatorEmail, String description) {
    this.uuid = uuid;
    if (!((creatorName.length() >= 6) && (creatorName.length() <= 64))) {
      throw new IllegalArgumentException("creatorName: (creatorName.length()>=6) && (creatorName.length()<=48)");
    }
    this.creatorName = creatorName;
    if (!((creatorEmail.length() >= 6) && (creatorEmail.length() <= 64))) {
      throw new IllegalArgumentException("creatorEmail: (creatorEmail.length()>=6) && (creatorEmail.length()<=48)");
    }
    this.creatorEmail = creatorEmail;
    if (!((description.length() >= 6) && (description.length() <= 64))) {
      throw new IllegalArgumentException("description: (description.length()>=6) && (description.length()<=48)");
    }
  ...
  public DomainHash createHash() {
    ...
    final String combination =
            MessageFormat.format("{0}:{1}:{2}", this.creatorName, this.creatorEmail, this.description);
